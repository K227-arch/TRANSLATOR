      ...
      <div className="flex flex-col md:flex-row gap-4 min-h-[360px]">
        ...
        <div className="flex-1 border border-outline-variant/30 rounded-2xl premium-shadow p-5 flex flex-col"
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            {input && (
              <button onClick={() => { setInput(""); setResult(null); resetFeedback(); }}
          ...
          <div className="mt-3 flex justify-between items-center border-t border-outline-variant/20 pt-3">
            <span className="text-xs text-outline">{input.length} / 5000</span>
            <button onClick={() => runSpellcheck(input)}
              className="p-1 text-outline hover:text-primary transition-colors" title="Spellcheck">
        ...
        <div className="flex-1 border-2 rounded-2xl premium-shadow p-5 flex flex-col overflow-y-auto"
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-1">
              {result?.translation && !feedbackSent && (
                <>
                  <button onClick={() => submitFeedback(1)}
                    className={`p-1.5 rounded-full transition-colors ${feedbackRating === 1 ? "text-primary bg-primary/15" : "text-outline hover:text-primary"}`}
                  ...
                  <button onClick={() => { setFeedbackRating(-1); setShowCorrection(true); }}
                    className={`p-1.5 rounded-full transition-colors ${feedbackRating === -1 ? "text-error bg-error/15" : "text-outline hover:text-error"}`}
              ...
              {feedbackSent && <span className="text-xs text-primary font-medium">Thanks!</span>}
              {result?.translation && (
                <button onClick={() => navigator.clipboard.writeText(result.translation || "")}
                  className="p-1.5 rounded-full text-outline hover:text-primary transition-colors" title="Copy">
          ...
          <div className="text-lg text-on-surface flex flex-col justify-start mb-3">
            {loading ? (
                ...
                <div className="flex space-x-1">
                  {[0, 1, 2].map(i => (
                    <div key={i} className="w-2 h-2 rounded-full animate-bounce"
                      style={{ background: "#e9c349", animationDelay: `${i * 0.15}s` }} />
                  ))}
                </div>
              ...
              <p className="text-outline/40 italic text-base">Translation will appear here...</p>
            )}
          </div>
          ...
          {(() => {
            ...
            if (norm(nllb) === norm(marian)) return null;
            return (
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="rounded-xl p-3 border transition-all"
                  style={{ background: "rgba(233,195,73,0.06)", borderColor: "rgba(233,195,73,0.2)" }}>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <span className="text-[10px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-full"
                      style={{ background: "rgba(233,195,73,0.15)", color: "#e9c349" }}>NLLB-200</span>
                ...
                <div className="rounded-xl p-3 border border-outline-variant/20 bg-surface-container/30 transition-all">
                  <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <div className="mt-2 flex gap-2 flex-wrap">
              {result.method === "neural_mt" && result.translation === result.translation_nllb && result.translation_nllb && !preferredModel && (
                <span className="text-xs px-2 py-0.5 rounded-full font-bold"
                  style={{ background: "rgba(233,195,73,0.12)", color: "#e9c349", border: "1px solid rgba(233,195,73,0.2)" }}>NLLB</span>
              )}
              {result.method === "neural_mt" && result.translation === result.translation_marian && !preferredModel && (
                <span className="text-xs bg-surface-container-high text-on-surface-variant px-2 py-0.5 rounded-full font-semibold">MarianMT</span>
              )}
              {preferredModel && (
                <span className="text-xs px-2 py-0.5 rounded-full font-bold"
                ...
                <div className="p-3 rounded-xl border border-outline-variant/30 space-y-2"
                    ...
                    {ERROR_TYPES.map(type => (
                      <div key={type}>
                        <label className="flex items-center gap-2 text-xs cursor-pointer">
                          <input type="checkbox" checked={errorTypes.includes(type)}
                          ...
                          <span className="text-on-surface-variant capitalize">
                            {type === "context" ? "Lack of context" : type === "vocabulary" ? "Word missing from vocab" : type}
                          </span>
                        </label>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => submitFeedback(-1)}
                      ...
                      className="text-xs bg-error text-on-error px-3 py-1.5 rounded-full hover:opacity-90 disabled:opacity-40 font-semibold">
                      Submit
                    </button>
                    <button
                      onClick={() => { setShowCorrection(false); setFeedbackRating(null); setErrorTypes([]); setCorrection(""); setOtherNote(""); }}
                ...
                <div className="pt-2 border-t border-outline-variant/20 space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {([
                      ...
                      <div key={id} onClick={() => setSelectedModel(id)}
                        className={`border rounded-xl p-2 cursor-pointer transition-all ${selectedModel === id ? "border-primary/60 bg-primary/10" : "border-outline-variant/30 hover:border-primary/30"}`}>
                        <div className="flex items-start gap-2">
                          <input type="radio" name="model_choice" checked={selectedModel === id}
                            onChange={() => setSelectedModel(id)} className="mt-0.5 flex-shrink-0 accent-primary" />
                  ...
                  {selectedModel && (
                    <button onClick={() => submitFeedback(selectedModel === "none" ? -1 : 1, selectedModel)}
                      className="w-full py-2 rounded-full text-xs font-bold hover:opacity-90 transition-all"
              ...
              {!benchmarkSent && (
                <div className="pt-2 border-t border-outline-variant/20">
                  <button onClick={() => setShowBenchmark(b => !b)}
                    className="flex items-center gap-1 text-xs text-on-surface-variant hover:text-on-background font-semibold transition-colors">
                  ...
                  {showBenchmark && (
                    <div className="mt-3 rounded-xl border border-outline-variant/30 p-3 space-y-3"
                      ...
                      <button onClick={submitBenchmark}
                        disabled={DIMS.every(d => dimScores[d.key] === null)}
                        className="w-full py-2 rounded-full text-xs font-bold hover:opacity-90 disabled:opacity-30 transition-all"
                        style={{ background: "#e9c349", color: "#1a1200" }}>
                        Submit Benchmark
                      </button>
                    </div>
      ...
      {tooltip && (
        <div className="fixed z-50 rounded-xl p-3 shadow-2xl min-w-[140px]"
          ...
          {tooltip.suggestions.length > 0 ? tooltip.suggestions.map(s => (
            <button key={s} className="block w-full text-left text-primary hover:bg-surface-container px-2 py-1.5 rounded text-sm transition-colors"
              onMouseDown={e => { e.preventDefault(); applySuggestion(tooltip.word, s); }}>{s}</button>
          )) : <p className="text-xs text-outline italic">No suggestions</p>}
          <div className="border-t border-outline-variant/20 mt-1 pt-1">
            <button className="text-xs text-outline hover:text-on-surface px-2 py-1 transition-colors"
              onMouseDown={e => { e.preventDefault(); ignoreWord(tooltip.word); }}>Ignore</button>
          </div>
        </div>
      )}
    </div>
