...
- Chat assistant powered by Llama 3.1 8B
...
- **Hallucination guard:** Detects and rejects degenerate MT output (repetitive tokens or bigrams exceeding 40%/35% thresholds) — falls back to the next translation method automatically. The secondary garbage check (`_is_garbage`) now flags only single-character token runs or outputs consisting entirely of one repeated token; the previous threshold-based short-word filter was removed because it incorrectly rejected valid short Runyoro sentences that naturally contain many 2–3-character particles (ni, na, ha, ku, mu, ne, …)
...
- **AI grammar review:** Sends editor text to `/chat` endpoint for Llama-powered grammar feedback
...
### Chat Assistant
- **LLM-powered:** Llama 3.1 8B via HuggingFace Router
...
- **Qwen refinement:** After the base translation, each label is passed through `_qwen_refine_translation()` to correct common noun-class agreement errors (prefix mismatches, R/L confusion) that occur with short noun-phrase inputs; the response includes both `label_lun` (refined) and `label_lun_raw` (unrefined base translation) for comparison or debug purposes; the `method` field carries a `+qwen` suffix when Qwen changed the label (e.g. `"nllb+qwen"`)
- **Supported formats:** JPEG, PNG, WebP — validated via magic byte detection (not Content-Type header)
- **File size limit:** 10 MB maximum
- **Label normalisation:** Raw ImageNet labels are mapped to clean common English nouns before translation via `_normalise_label()` and the `_LABEL_NORMALISE` lookup table in `image_classifier.py`. This covers ~80 verbose or scientific ImageNet categories across animals (elephants, big cats, dogs, birds, primates, reptiles, bovines, other wildlife), plants (banana, pineapple, jackfruit, fig), vehicles (scooters, vans, emergency vehicles), and common objects (phones, laptops, screens). Labels not in the table pass through unchanged, ensuring the MT models receive natural common nouns rather than breed names or scientific synonyms.
- **LLM label verification:** After static normalisation, labels are passed through `_qwen_verify_image_label()` in `main.py`, which asks the LLM (configured via `HF_CHAT_MODEL`, default `meta-llama/Llama-3.1-8B-Instruct`) to confirm or simplify the label into the single most accurate common English noun (1–3 words max). This catches cases where the static lookup table has no entry and MobileNetV2 returns a verbose or misclassified ImageNet label (e.g. `"african elephant, Loxodonta africana"` → `"elephant"`). Requires `HF_TOKEN` to be set; falls back to the raw label silently on any error or if the LLM response is too long (> 4 words).
- **Top-K results:** Returns the top 1 prediction sorted by confidence descending; the frontend requests `top_k=1` so only the single highest-confidence label is shown; raw ImageNet labels are first normalised via `_normalise_label()` before translation
...
- **Classification result card:** After object identification (Identify tab), results are shown as a surface card with an `image_search` icon header labelled "Object Identified". The card body displays the Runyoro label (`label_lun`) in large bold primary-coloured text and the English label (`label_en`) in a smaller muted line below it. No confidence score or progress bar is rendered — the card focuses solely on the translated object name.
...
- **Error handling:** Checks for camera API availability before attempting access (catches insecure-context / unsupported-browser scenarios); displays camera permission errors and server connection failures in a centred styled error banner; if the first `getUserMedia` attempt fails with a permission-denied error (message contains `denied`, `permission`, `not allowed`, or `notallowederror`), the camera launch skips the generic-constraints retry and immediately falls back to the native file picker — avoiding a redundant second permission prompt on browsers that have already blocked the camera
...
> **Mobile / Android emulator access:** `next.config.ts` sets `allowedDevOrigins` to `["10.0.2.2", "10.0.2.2:3002"]`, so the Next.js dev server accepts requests from Android emulators (which route `10.0.2.2` to the host machine) and from other devices on your local network. No extra config is needed when testing the frontend from an emulator or a physical device on the same Wi-Fi.
...
> **Logging:** The backend uses Python's standard `logging` module with structured output (`%(asctime)s [%(name)s] %(levelname)s: %(message)s`). The default level is `INFO`. Override it with the `LOG_LEVEL` environment variable:
> ```bash
> LOG_LEVEL=DEBUG python main.py   # verbose — shows model load details, request paths, etc.
> LOG_LEVEL=WARNING python main.py # quieter — only warnings and errors
> ```
...
### Utilities
- `POST /summarize-pdf` — Extract + translate + summarize documents. When a Lunyoro document is detected, grammar rules (nasal assimilation, particle elision, kinship correction, copula normalization) are applied to each sentence before translation. Llama 3.1 8B (if `HF_TOKEN` is set) refines **both** the MarianMT and NLLB-200 drafts independently, with Grammar Rules 4 post-processing applied to each refined output. The best result (NLLB-refined preferred, Marian-refined as fallback) is returned as `summary_lunyoro`. All four variants are included in the response: `summary_lunyoro` (best), `summary_lunyoro_marian` (Marian-refined), `summary_lunyoro_nllb` (NLLB-refined). Falls back to the MT draft per model if Llama is unavailable.
...
The frontend is a Progressive Web App (PWA) powered by `next-pwa` / Workbox (`next.config.ts`). The service worker is **enabled in production only** — it is disabled in development to prevent repeated `sw.js` regeneration from triggering full page reloads on every HMR update.
...
- **`/translate-reverse` improvement:** Added optional `refine: bool` parameter (default `false`). When `true` and `HF_TOKEN` is set, a Qwen 2.5 7B pass refines the lun→en MT draft for fluency, accuracy, and natural English phrasing before the response is returned. The refinement runs in a background thread with a 10s hard timeout (8s per-request timeout inside the thread) so the worker is never blocked beyond that even if the HF Router is slow or unavailable — timeouts and errors are logged at DEBUG level and the raw MT output is returned unchanged. The refined output is also recorded in translation history with a `+refined` method suffix.
